...
Codex usage is `$council [core|auto|core-eng|core-ux|core-mix|all|debate] <problem-description|@file>`. `core` remains the default; `auto` is explicit and selects exactly 6 best-fit reviewers. Codex Council spawns native reviewer agents directly from installed Codex agent definitions, passes a bounded review bundle, validates each finished report, strips transport markers, and synthesizes disagreement. Reviewers may read directly connected files when needed, but must not wander the repo or run tests/builds. Runs broader than 6 reviewers require explicit current-run approval; if approval is unavailable, Codex Council stops with exactly `Council not run: broad council approval not granted.` While reviewers are active, the Codex orchestrator stays alive, performs per-reviewer health checks at least once per minute, nudges drifting or stalled reviewers with `followup_task`, and may emit sparse `Council progress:` updates.

Codex council development follows the repo `AGENTS.md` loop: commit the implementation first with `git commit -sS`, push the signed commit, keep the `sai` marketplace configured as the Git marketplace, run `codex plugin marketplace upgrade sai`, verify the installed cache contains the bumped version and changed `skills/council` text, then run the live improvement loop from the real target repository using cheap-model smoke checks before escalating. Do not validate by switching `sai` to a local marketplace path; local-path registration can leave the versioned cache stale. The strict inline-or-explicit-file boundary and exact broad-run denial sentence must be present in both the skill frontmatter and `.codex-plugin` manifest because Codex may perform routing or memory setup before opening the full `SKILL.md`. Enabled under-development Codex agent features such as `multi_agent_v2`, `enable_fanout`, `child_agents_md`, and `runtime_metrics` are part of that validation surface unless the improvement loop proves a negative council impact.
