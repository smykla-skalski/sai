...
Formal Council improvement process rule: working behavior comes before score. First prove the behavior through real installed-plugin runs, even if the prompt is verbose, token-expensive, or plugin-eval scores are lower. Only after that proof exists should a later iteration compress wording and improve score/token footprint.
