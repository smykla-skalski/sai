...
Codex council development follows the repo `AGENTS.md` loop: commit the implementation first with `git commit -sS`, install or upgrade the bumped local Codex plugin, verify the installed cache contains the bumped version and changed skill text, then run the live improvement loop from the real target repository using cheap-model smoke checks before escalating.
