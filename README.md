...
|:-----------------------|:---------------------------------------------------------------------------------|:-----------------------------|
| **council**            | Run native Codex reviewer-agent councils and synthesize concrete next moves | `plugins/council/codex-skills/council/` |
...
Codex council development follows the repo `AGENTS.md` loop: commit the implementation first with `git commit -sS`, install or upgrade the bumped local Codex plugin, verify the installed cache contains the bumped version and changed `codex-skills/council` text, then run the live improvement loop from the real target repository using cheap-model smoke checks before escalating. Enabled under-development Codex agent features such as `multi_agent_v2`, `enable_fanout`, `child_agents_md`, and `runtime_metrics` are part of that validation surface unless the improvement loop proves a negative council impact.
...
[Claude documentation ->](./claude/council/README.md) · [Codex skill ->](./plugins/council/codex-skills/council/SKILL.md)
