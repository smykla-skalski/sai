...
1. Run server-side dry-run with `bash "$SKILL_DIR/scripts/validate-manifest.sh"`.
...
```bash
make build/kumactl
KUMACTL="$(bash "$SKILL_DIR/scripts/find-local-kumactl.sh")"
"${KUMACTL}" version
```
...
Fix: use `bash "$SKILL_DIR/scripts/cluster-lifecycle.sh"`, which auto-generates a temporary manifest for numeric names `kuma-<n>`.

## Failure triage checklist

When a test fails:

1. Run `bash "$SKILL_DIR/scripts/capture-state.sh"` immediately with label `failure-<test-id>`.
