...
if [[ "${has_oversized}" == "true" ]]; then
  jq -nc '{
    hookSpecificOutput: {
      ...
      additionalContext: "[KSA007] Code-reading agent may have returned raw code instead of structured summary. Found code blocks exceeding 20 lines. Review the output - it should contain only G1-G7 material entries and S1-S7 signal entries."
    }
  }'
fi
