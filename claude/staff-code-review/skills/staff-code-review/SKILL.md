...
Before spawning the subagent, resolve the two reference file paths yourself — a
spawned `general-purpose` subagent has no `CLAUDE_SKILL_DIR` of its own, so a
literal `${CLAUDE_SKILL_DIR}`-templated path in its prompt cannot resolve.
Search bounded to the plugin cache root, never the whole filesystem:

```
find "${CLAUDE_SKILL_DIR}/../../../.." -maxdepth 6 -path "*/humanize/*/skills/humanize/references/patterns.md" 2>/dev/null | head -1
find "${CLAUDE_SKILL_DIR}/../../../.." -maxdepth 6 -path "*/humanize/*/skills/humanize/references/elements-of-style.md" 2>/dev/null | head -1
```

If either search returns nothing (humanize plugin not installed, or its
install layout differs), skip this Humanize Comments pass entirely and keep
the synthesized review as-is — do not widen the search beyond the plugin
cache root.

Spawn a single `Agent` (subagent_type: "general-purpose") with this prompt,
substituting the two absolute paths you just resolved (not the
`${CLAUDE_SKILL_DIR}` template):
...
PATTERN CATALOG: Read the file at <resolved absolute path to patterns.md>
COMPOSITION GUIDE: Read the file at <resolved absolute path to elements-of-style.md>
