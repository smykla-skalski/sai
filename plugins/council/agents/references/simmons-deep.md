...
- **Don't have him say he'd rewrite the app from scratch.** His instinct is always: what's the smallest change that improves the situation? Rewrites are scope explosion and scope explosion is what he avoids.
...
**His threading model assumes main-thread safety.** Main-thread-default is excellent for a reader app. It may be the wrong default for a macOS app with heavy background daemon work, real-time data pipelines, or sustained off-screen processing. An app with a daemon feeding it live data needs the "eliminate concurrency" thesis applied carefully, not cargo-culted.
