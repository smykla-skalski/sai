...
description: >-
  ...
  silently. Before spawn, inspect native agent state and clean stale Council
  children. If state is clean/root-only, run the largest safe reviewer wave,
  capped at 3. Use high reasoning, complete material per reviewer, visible
  progress only as `Council progress:`, never solo or raw child output.
---
...
4. Prepare agent capacity before any spawn. First inspect native live-agent state. Prefer `list_agents` with no args when the tool is exposed; otherwise use the native agent-state signal the runtime provides. Never use shell/command execution for live-agent state. Do not run `rtk ls_agents`, `harness`, `ps`, `pgrep`, `pwd`, `ls`, `find`, or `rg` for Council orchestration. `path_prefix` only for known `/root/...` agent paths, never filesystem paths. Ignore `/root`. Close stale Council reviewer children first, wait for every close result, then re-check state. If state is clean/root-only, emit exactly `Council progress: agent state clean: root only; running largest safe wave.` and use the largest safe wave capped at 3. If live non-Council children remain or capacity is constrained, emit the reason and use smaller waves. Do not spawn into a known full session. Respect session thread limits.
5. Fan out in waves sized by cleaned capacity. Max 3 `spawn_agent`s per batch; six-reviewer roster is exactly two waves of up to 3 after clean/root-only state. Wave order is strict: spawn wave N, wait until every wave-N reviewer is `accepted/missing/failed`, close every wave-N reviewer, observe the close results, then spawn wave N+1 if any selected slug remains. Closing is a hard tool-call barrier: never queue or attempt a next-wave `spawn_agent` until all wave-N `close_agent` calls have completed. Never synthesize after wave 1 of a six-reviewer roster. Spawn with `spawn_agent(agent_type: "<slug>", fork_turns: "none", reasoning_effort: "high")` when the schema accepts `reasoning_effort`; if role-managed agents reject overrides, rely on the installed reviewer manifest's high-effort setting and do not set any medium/low fallback. Keep the model unchanged unless the user requested a model change.
