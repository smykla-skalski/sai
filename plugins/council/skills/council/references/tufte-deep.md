...
The `artifact-*` and `interaction-*` findings the council reviews are exactly his territory. A dashboard of agent sessions, task states, performance metrics, and real-time status is precisely his domain. Every design decision on that screen is a data-ink decision. Specific mappings:
