...
for pattern in ${GENERIC_PATTERNS[@]+"${GENERIC_PATTERNS[@]}"}; do
  ...
  if [[ -n "$MATCH" ]]; then
    ...
    ESCAPED_LINE_TEXT=$(json_escape "$LINE_TEXT")
    echo "{\"check\": \"generic-advice\", \"pass\": false, \"detail\": \"Found: '${ESCAPED_PATTERN}' on line ${LINE_NUM}: ${ESCAPED_LINE_TEXT}\"}"
  ...
  if [[ "$line" =~ ^\`\`\` ]]; then
    if [[ "$IN_BLOCK" -eq 0 ]]; then
      ...
      if [[ "$BLOCK_LINES" -gt 10 ]]; then  # >10 lines = should be a file:line ref instead
        LONG_BLOCK_DETAIL="Code block starting at line ${BLOCK_START} is ${BLOCK_LINES} lines (>10)"
        echo "{\"check\": \"long-code-blocks\", \"pass\": false, \"detail\": \"${LONG_BLOCK_DETAIL}\"}"
...
if [[ "$TOTAL" -eq 0 ]]; then
  ...
  if [[ "$RATIO" -gt 60 ]]; then  # >60% bullets = good structure per Anthropic best practices
    BULLET_RATIO_DETAIL="Bullet ratio ${RATIO}% (${BULLET_LINES} bullet lines, ${PARA_LINES} paragraph lines)"
    echo "{\"check\": \"bullets-vs-paragraphs\", \"pass\": true, \"detail\": \"${BULLET_RATIO_DETAIL}\"}"
  else
    BULLET_RATIO_DETAIL="Bullet ratio ${RATIO}% (${BULLET_LINES} bullet lines, ${PARA_LINES} paragraph lines) — prefer >60%"
    echo "{\"check\": \"bullets-vs-paragraphs\", \"pass\": false, \"detail\": \"${BULLET_RATIO_DETAIL}\"}"
  fi
fi
